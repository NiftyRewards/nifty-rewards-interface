...
function getInOrOut(inputs: any) {
  ...
  for (let i = 0; i < inputs.length; i += 1) {
    const ioType: string = inputs[i].type;
    if (ioType === "string" || ioType === "bytes" || ioType.endsWith("[]")) {
      out += `${inputs[i].type} memory`;
    } else {
      out += inputs[i].type;
    }
...
export function ABI2SolidityFiles({ input, output }: any) {
